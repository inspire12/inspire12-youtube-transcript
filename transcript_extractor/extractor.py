...
def get_transcript(video_id, translate=False):
    ...
    transcript_list = youtube_transcript_api.fetch(video_id=video_id, languages=['ko'])
    lines = []
    translator = Translator() if translate else None
    base_url = f"https://www.youtube.com/watch?v={video_id}"
    for entry in transcript_list:
        start_sec = int(entry.start)
        timestamp = format_timestamp(int(entry.start))
...
def save_to_markdown(metadata, transcript):
    ...
    """
    file_name = make_filename(metadata['title'])
...
def run(raw_input_value):
    ...
    return make_filename(metadata['title'])
